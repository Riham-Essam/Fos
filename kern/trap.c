...
void placement_Fault_Hndler(struct Env * curenv, uint32 fault_va);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	uint32 Victime_ModifiedClock;
	fault_va = ROUNDDOWN(fault_va, PAGE_SIZE);
	int max_size_page = curenv->page_WS_max_size;
	int size_of_workingset = env_page_ws_get_size(curenv);
	int chek_permission;
	uint32 *ptr = NULL;
	uint32 victim_address;
	struct Frame_Info *frame_info_ptr = NULL;

//if size is smaller than max size of the working set that is allocated and search in stack

	if (size_of_workingset < max_size_page) {

		placement_Fault_Hndler(curenv, fault_va);
	}
		...
		//It works by two try
		//first, search in all working set of any address that is used and modified = 0 to be the victim
		//second, search for all used bit and set the by 0 and search for modified that equal not modified to be a victim

		if (isPageReplacmentAlgorithmModifiedCLOCK()) {

			//if there is no space, it will remove page that is not important

			int check = 0;

			//variable to save last index

			uint32 last_index = curenv->page_last_WS_index;
			uint32 victim = env_page_ws_get_virtual_address(curenv, last_index);

			Tryfirst: for (int k = 0; k < max_size_page; k++) {

				victim = env_page_ws_get_virtual_address(curenv, last_index);

//search for all permused and permmodified = 0 and save this address to be a victim

				chek_permission = pt_get_page_permissions(curenv, victim);
				if ((chek_permission & PERM_MODIFIED) == 0
						&& (chek_permission & PERM_USED) == 0)

						{
					Victime_ModifiedClock = victim;
					goto Trythird;
					break;

				}
				last_index++;
				if (last_index == curenv->page_WS_max_size) {

					last_index = 0;

				}
			}
			Trysecond: for (int k = 0; k < max_size_page; k++) {
//to set all permused in the working set = 0

				victim = env_page_ws_get_virtual_address(curenv, last_index);
				chek_permission = pt_get_page_permissions(curenv, victim);
				if ((chek_permission & PERM_USED) == PERM_USED) {

					pt_set_page_permissions(curenv, victim, 0, PERM_USED);
				} else {
				...
				//change address to show next address and check if is reached max, change it to show first address
			...
			//to replace
			Trythird: victim_address = env_page_ws_get_virtual_address(curenv,
					last_index);

			chek_permission = pt_get_page_permissions(curenv, victim_address);

			//check if modified bit equal not modified and get frame of this address and update working set and unmap frame

			if ((chek_permission & PERM_MODIFIED) == PERM_MODIFIED) {

				uint32 * ptr = NULL;
				frame_info_ptr = get_frame_info(curenv->env_page_directory,
						(void*) victim_address, &ptr);

				pf_update_env_page(curenv, (void*) victim_address,
						frame_info_ptr);

				unmap_frame(curenv->env_page_directory, (void*) victim_address);

			} else {

				//if the is no space it will unmap frame

				unmap_frame(curenv->env_page_directory, (void*) victim_address);

			}

			env_page_ws_clear_entry(curenv, last_index);

			//to allocate frame and check it not NULL
...
void placement_Fault_Hndler(struct Env * curenv, uint32 fault_va) {
	...
	struct Frame_Info *free_frame = NULL;
	int return_allocation = allocate_frame(&free_frame);

	if (return_allocation != E_NO_MEM) {

		// to map frame

		map_frame(curenv->env_page_directory, free_frame, (void *) fault_va,
		PERM_WRITEABLE | PERM_USER | PERM_PRESENT);

	}
	// to read  page file
	int return_read = pf_read_env_page(curenv, (void*) fault_va);

	if (return_read == E_PAGE_NOT_EXIST_IN_PF) {

		//to search if is exist in stack or not

		if (fault_va < USTACKTOP && fault_va >= USTACKBOTTOM) {

			int return_add_empty = pf_add_empty_env_page(curenv, fault_va, 0);

			if (return_add_empty == E_NO_PAGE_FILE_SPACE) {
				panic("No Enough Virtual Space in Page File");
			}
...
// to reflect to working set and set for index free

	for (int q = 0; q < max_size_page; q++) {

		if (curenv->ptr_pageWorkingSet[q].empty) {

			curenv->page_last_WS_index = q;

			break;
		}
	}

	//to set entry and increment address and make address to show to start

	env_page_ws_set_entry(curenv, curenv->page_last_WS_index, fault_va);
	curenv->page_last_WS_index++;

	//to go to first element in stack if reach to max size in working set

	curenv->page_last_WS_index = curenv->page_last_WS_index % max_size_page;
}
