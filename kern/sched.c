...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel) {
	...
	uint8 numberOfGivenLevels = numOfLevels;
	...
	for (int LvlItr = 0; LvlItr < numberOfGivenLevels; LvlItr++) {
		init_queue(&env_ready_queues[LvlItr]);
		quantums = kmalloc(num_of_ready_queues * sizeof(uint8));
	}
	//[2] Create the "quantums" array and initialize it by the given quantums in "quantumOfEachLevel[]"

	for (int levelItr = 0; levelItr < numberOfGivenLevels; levelItr++) {
		quantums[levelItr] = quantumOfEachLevel[levelItr];
	}
...
//[1] If the current environment (curenv) exists, place it in the suitable queue
	int k = 0;
	int queue_ready = num_of_ready_queues;
	struct Env *env_sechduler = NULL;

	if (curenv != NULL) {
		if (level_secheduler == queue_ready - 1) {
			level_secheduler = queue_ready - 1;
			enqueue(&env_ready_queues[level_secheduler], curenv);

		} else if (level_secheduler < queue_ready) {
			level_secheduler = level_secheduler + 1;
			enqueue(&env_ready_queues[level_secheduler], curenv);
	...
	while (k < queue_ready) {
		uint32 queue_scheduler = queue_size(&env_ready_queues[k]);

		if (queue_scheduler > 0) {
			...
			env_sechduler = dequeue(&env_ready_queues[k]);

			kclock_set_quantum(quantums[k]);

			level_secheduler = k;

			return env_sechduler;
