...
void * create_page_table(uint32 *ptr_page_directory,
	...
	uint32* new_page_table = kmalloc(PAGE_SIZE);
	//se CONSTRUCT_ENTRY() to build the table entry then link it to the given directory

	if (new_page_table != NULL) {

		uint32* physical_of_table = (uint32*) kheap_physical_address(
				(uint32) new_page_table);

		uint32* virtual_of_table = &ptr_page_directory[PDX(virtual_address)];

		*virtual_of_table = CONSTRUCT_ENTRY((uint32 )physical_of_table,
				PERM_USER|PERM_PRESENT| PERM_WRITEABLE| PERM_MODIFIED);

		//	a.	clear all entries (as it may contain garbage data)

		for (int entries = 0; entries < 1024; entries++)
			new_page_table[entries] = 0;

		//	b.	clear the TLB cache (using "tlbflush()")

		tlbflush();

		return (void *) new_page_table;
	}
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	int NumberOfFrames = size / PAGE_SIZE;

	uint32 currVAAddr = virtual_address;

	for (int frameItr = 0; frameItr < NumberOfFrames; frameItr++) {
		int result = pf_add_empty_env_page(e, currVAAddr, 0);

		if (result == E_NO_PAGE_FILE_SPACE) {
			panic(
					"ERORR FROM SYS_MALLOC: THERE IS NO ENOUGH SPACE IN PAGE FILE!!!.\n");
		}

		currVAAddr += PAGE_SIZE;
	}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	uint32 max_working_set = e->page_WS_max_size;
	uint32 virtual_working_address;

	uint32 givenFNumber = size / PAGE_SIZE;
	uint32 givenVA = virtual_address;

	struct Frame_Info* fInfoPtr = NULL;
	int frameItr = 0;
	int working_set = 0;

	while (frameItr < givenFNumber) {

		//To remove existing environment page at the given virt addr from the page
		pf_remove_env_page(e, givenVA);

		//To go to next virt addr
		givenVA += PAGE_SIZE;
		frameItr++;
	}

	//To get actual addr after changes
	givenVA = virtual_address;

	//2. Free ONLY pages that are resident in the working set from the memory

	while (working_set < max_working_set) {

		//to get virtual address for all working set
		virtual_working_address = env_page_ws_get_virtual_address(e,
				working_set);

		//to show if virtual address of page exsist in working set or no

		if ((virtual_working_address >= givenVA)
				&& (virtual_working_address < (givenVA + (size)))) // to get correct range

				{

			//to remove from memory
			unmap_frame(e->env_page_directory, (void*) virtual_working_address);

			//remove page from working set
			env_page_ws_clear_entry(e, working_set);
	...
	//Remove not present page tables
	//Don't remove present one

	uint32* ptrToBeRemoved = NULL;
	uint32 present_bitVA;
	givenVA = ROUNDDOWN(virtual_address, (1024*PAGE_SIZE));
	uint32 frameItrate = givenVA;
	int entryItr = 0;

	while (frameItrate < (size + givenVA)) {
		get_page_table(e->env_page_directory, (void*) frameItrate,
				&ptrToBeRemoved);

		int found = 1;

		if (ptrToBeRemoved != NULL) {

			while (entryItr < 1024) {

				present_bitVA = ptrToBeRemoved[entryItr] & PERM_PRESENT;
			...
			if (found) {
				...
				//Page table will be removed
				kfree((void*) ptrToBeRemoved);

				//page dir to be freed
				pd_clear_page_dir_entry(e, (uint32) frameItrate);
			}

		}
		frameItrate += (PAGE_SIZE * 1024);
	}

//to refresh cash memory
	tlbflush();
